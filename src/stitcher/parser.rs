...
pub fn modify_playlist(mut playlist: Playlist, session_id: &str, base_url: &str, origin_url: &str) -> Result<String, Box<dyn std::error::Error>> {
  ...
  if let Playlist::MediaPlaylist(ref mut media_playlist) = playlist {
    for segment in &mut media_playlist.segments {
      ...
      let segment_name = if segment.uri.starts_with("http") {
        segment.uri.split('/').last().unwrap_or(&segment.uri)
      } else {
        &segment.uri
      };
      ...
      let new_uri = format!("{}/stitch/{}/segment/{}?origin={}",
        base_url,
        session_id,
        segment_name,
        origin_url
